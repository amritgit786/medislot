...
// API for to update user profile data
  ...
  try {
    ...
    if (slots_booked[slotDate]) {
      if (slots_booked[slotDate].includes(slotTime)) {
        return res.json({ success: false, message: "Slot Time not available" });
      } else {
    ...
    res.json({ success: true, message: "Booked Appointment Successfully" });
  } catch (error) {
    console.log(error);
    res.json({ success: false, message: error.message });
  }
...
// API to paid a payment for an appointment using razorpay
